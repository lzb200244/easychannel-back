...
class GroupRecords(BaseModel, models.Model):
    """
    群聊天记录表
    """
    # 消息类型
    ...
    user = models.ForeignKey(UserInfo, on_delete=models.CASCADE,
                             verbose_name='发言人', )
    # 自关联
    room = models.ForeignKey('GroupRoom', on_delete=models.CASCADE, verbose_name='聊天室')
    file = models.JSONField(blank=True, null=True, verbose_name="文件信息")
    ...
    class Meta:
        # 表名
        db_table = 'chat_group_records'


class GroupRoom(BaseModel, models.Model):
    """
    聊天室模型
    """
    ...
    class Meta:
        ...
        db_table = 'chat_group_room'

    def save(self, *args, **kwargs):
        if not self.id:
            self.id = snowflake.generate_id()
        super(GroupRoom, self).save(*args, **kwargs)
...
"""
class PrivateRecord(models.Model):
